# Cloud Computing Demonstration Series

This repository contains a series of demonstrations covering cloud computing technologies including AWS, Google Cloud Platform (GCP), containerization, machine learning, microservices, monitoring, and CI/CD automation.

These demonstrations were developed as part of a cloud computing coursework project and illustrate how modern cloud applications can be deployed, monitored, and automated using industry tools and platforms.

---

## Weekly Demonstrations

| Week | Topic | Platform | Video |
|------|------|----------|-------|
| Week 1 | Cloud Environment Setup | AWS / GCP | [Watch Video- AWS](https://youtu.be/4wdPsm9KpWk?si=szPl2SFvv2Qdhui_), [GCP](https://youtu.be/H5Mvf3dPxB4?si=GIV56-30k9vmFOYQ)|
| Week 2 | GitHub Project Deployment | AWS / GCP |[Watch Video- AWS](https://youtu.be/ESWNkE1GqUo?si=TTY46ARIp6vRmsE4), [GCP](https://youtu.be/3HGFHE5aO4c?si=2WEc4fL9My_6ULLF)|
| Week 3 | Containerized Application Deployment | AWS / GCP |[Watch Video- AWS](https://youtu.be/7djW0h7JYCM?si=RVxnoe0Xk-nLfFX3), [GCP](https://youtu.be/5kKbcufKR_g?si=N1GVIO9DmxokWCU4)|
| Week 5 | Machine Learning with BigQuery ML | GCP |[Watch Video](https://youtu.be/pRbv0rju4hw?si=DwRb1KJ8MSK8YDbs)|
| Week 6 | AutoML Application with BigQuery ML | GCP |[Watch Video](https://youtu.be/3WPIZXEILkU?si=h52fzKESKW9jjv8N)|
| Week 7 | Containerized Microservice Application (Golang) | Cloud |[Watch Video](https://youtu.be/j_dpFsQ_WsE?si=GLeuAYWRKDXNOTkb)|
| Week 8 | Containerized Pub/Sub Microservice Architecture | GCP |[Watch Video](https://youtu.be/fqWgdhMzvrY?si=O1mEk7hR0bDjhm9w)|
| Week 9 | Application Monitoring with Prometheus and Grafana | Cloud |[Watch Video](https://youtu.be/W2DJwvJJANI?si=7kXsJkrArMKKfGjK)|
| Week 10 | CI/CD Automation with GitHub Actions | Cloud |[Watch Video](https://youtu.be/pCgYyAvBTgU?si=OaOrVrQ7dMTFncsn)|

---

## Technologies Demonstrated

The demonstrations include the following cloud and development technologies:

- Amazon Web Services (AWS)
- Google Cloud Platform (GCP)
- Docker containerization
- Microservice architectures
- Pub/Sub messaging patterns
- Google BigQuery and BigQuery ML
- Prometheus and Grafana monitoring
- CI/CD automation with GitHub Actions

---

## Purpose

These demonstrations illustrate key concepts in cloud-native application development, including containerized services, distributed system communication, machine learning in the cloud, application monitoring, and automated development workflows.
